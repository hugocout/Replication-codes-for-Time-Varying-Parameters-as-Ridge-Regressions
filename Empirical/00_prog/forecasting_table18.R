...
POOS <- function(it_pos) {
  ...
  source(paste(paths$fun, 'MV2SRR_v221103.R', sep='/'))
  ...
  sname = paste(paths$out,'/table16to18/TVPfcst_',all_options$V[it_pos],h,mod,'_block.RData',sep='')
  ...
  for (v in all_options$V[it_pos]){ #all vars
    ...
    for(t in tau:bigt){
      ...
      bb = tvp.ridge(Y=train[subset,1],X=train[subset,-1],block_size = 8,CV.plot = F,
                     lambda.candidates =lambdavec,lambda2=CV$lambda.min,
                     oosX=last[-1])
      
      m=2
      forecast[t,h,v,m] <- OF(bb$forecast) -last[1]
      hp_track[t,h,v,m,1]= CV$lambda.min 
      hp_track[t,h,v,m,2]= bb$lambdas
