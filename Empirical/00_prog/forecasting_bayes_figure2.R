...
POOS <- function(it_pos) {
  ...
  source(paste(paths$fun, 'TVPRR_v181111_221103.R', sep='/'))
  ...
  sname = paste(paths$out,'/table16to18/TVPfcst_',all_options$V[it_pos],h,mod,'_bayes.RData',sep='')
  ...
  for (v in all_options$V[it_pos]){ #all vars
    ...
    for(t in tau:bigt){
      ...
      if(mod!=4){killingit = 0}
      else{killingit=1}
      mdl = glmnet(x=train[subset,-1],y=train[subset,1],  family='gaussian', alpha=0,lambda=CV$lambda.min*killingit)
      ...
      # 2. Bayesian -------------------------------------------------------
      data=data.frame(y=train[subset,1],train[subset,-1])
      res <- shrinkTVP(y ~ ., data = data,mod_type = 'triple', niter=5000,
                       sv = TRUE)
      
      betas.bayes=rep(NA,ncol(train))
      for(kkk in 1:(ncol(train)+1-1)){
        betas.bayes[kkk]=mean(res$beta[[kkk]][,nrow(train)])
      } 
      
      m=2
      forecast[t,h,v,m] <- OF(c(1,last[-1])%*%betas.bayes) -last[1]
