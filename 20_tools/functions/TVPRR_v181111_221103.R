TVPRR <-function(X,y,type,lambdavec=exp(linspace(-6,20,n=50)),lambda2=.1,olsprior=0,
  ...
  cvlist <- cv.univariate(ZZ=ZZ,Y=yy,dimX = dimX,lambdavec = lambdavec,k=kfold,plot=abs(silent-1),sweigths=sweigths)
  ...
  if(TYPE==2){
    ...
    #cvlist <- cv.univariate(ZZ=ZZ,Y=yy,dimX = dimX,lambda2 = lambda2,
  ...
  if(2<TYPE & TYPE<6){
    ...
    repeat {
      ...
      #stack <- cv.univariate(ZZ=ZZ,Y=yy,dimX = dimX,lambda2=lambda2,
    ...
    cvlist <- cv.univariate(ZZ=ZZ,Y=yy,dimX = dimX,
  ...
  if(TYPE==6){
    ...
    cvlist <- cv.univariate(ZZ=aa$Zrot,Y=yy,dimX = dimX,
                           lambdavec = lambda1,lambda2=lambda2,
                           k=kfold,plot=1,sweigths=SW)
