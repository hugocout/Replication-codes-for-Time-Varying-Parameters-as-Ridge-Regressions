...
{    
  ...
  cons = 0.5*rnorm(howmanytvp)/sqrt((M/20))
  cons[2]=0.5
  
  for(t in 2:obs) {
    for(j in 1:howmanytvp){
      if(j>fracvec[frac]*howmanytvp){beta[t,j]=cons[j] +compressor*(sin(t/20) + sig_heta*heta[t,j])
      truesig[j] = 1
