dualGRR <-function(Zprime,y,dimX,lambda1,lambda2=0.001,CI=0,sweigths=1,olsprior=1,eweigths=1,GCV=0,calcul_beta=1,nf=0){
  ...
  Kmat_half[(ncolZ-dimX+1):ncolZ,]=(1/lambdas[2])*Kmat_half[(ncolZ-dimX+1):ncolZ,] #* matrix(rexp(length((ncolZ-dimX+1):ncolZ)*ncol(Kmat_half)),length((ncolZ-dimX+1):ncolZ),ncol(Kmat_half)) 
  ...
  if(olsprior==1){
    ...
    #beta_ols = beta_ols + rnorm(length(beta_ols),mean=0,sd=diag(solve(crossprod(X))*sd(y-X%*%beta_ols)))
