...
tvp.ridge <-function(X,Y,lambda.candidates=exp(linspace(-6,20,n=15)),oosX=c(),
                      lambda2=.1,kfold=5,CV.plot=TRUE,CV.2SRR=TRUE,block_size=8,
  ...
  dimX= dim(as.matrix(X))[2]+1 # + constant
    ...
    LAMBDAS = rep(0,M)
    SIGMA.U = array(NA,dim=c(M,ncol(X)+1))
    ...
    YHAT_VARF=Y
      ...
      if(length(lambdavec)>1){
        ...
        cvlist <- CV.KF.MV2(ZZ=ZZ,Y=Y,dimX = dimX,lambdavec = lambdavec[l.subset],lambda2=lambda2,
                           k=kfold,plot=abs(silent-1),sweigths=sweigths,block_size = block_size) #,eweigths = abs(yy)/mean(abs(yy)))
    ...
    for(m in 1:M){
      ...
      #Final estimation
      grr<-dualGRR2(ZZ,Y[,m],dimX,lambda1,olsprior = olsprior,lambda2=lambda2,sweigths=sweigths) #,eweigths = sqrt(abs(Y[,m]))/mean(abs(Y[,m])))
      ...
      if(CV.again){cvlist <- cv.univariate2(ZZ=ZZ,Y=Y[,m],dimX = dimX,lambda2 = lambda2,eweigths = EW,block_size=block_size,
                                                     lambdavec = lambdavec,k=kfold,plot=abs(silent-1),sweigths=sigmasq)
      usethislambda1=cvlist$minimizer
      LAMBDAS[m]=usethislambda1
      }
      else{usethislambda1=lambda1}

      if(homo.param>0 | sv.param>0){
      grrats<-dualGRR2(ZZ,Y[,m],dimX,lambda1=usethislambda1,eweigths = EW,olsprior = olsprior,
  ...
  return(list(betas.rr=BETAS_GRR,betas.2srr=BETAS_VARF,
              lambdas=LAMBDAS,forecast=fcast,sig.u = SIGMA.U,
...
dualGRR2 <-function(Zprime,y,dimX,lambda1,lambda2=0.001,CI=0,sweigths=1,olsprior=1,eweigths=1,GCV=0,calcul_beta=1,nf=0){
  ### Inputs are: ####
  ...
  for(m in 1:(nf)){
    ...
    # expmat = matrix(rexp(length(begin:end)*ncol(Kmat_half)),length(begin:end),ncol(Kmat_half)) #*12000
    # Kmat_half[begin:end,]=Kmat_half[begin:end,]*expmat
    # 
...
cv.univariate2 <- function(Y,ZZ,k,lambdavec,lambda2=0.001,dimX,plot=0,sweigths=1,eweigths=1,nf=dimX,block_size=8){
  ...
  if(block_size==1){index <- sample(1:k, nrow(data), replace = TRUE)}
  else{ 
    # index <- sample(1:k, nrow(data), replace = TRUE)
    # sample_index <- c(1:nrow(data))
    # groups<-sort(base::sample(x=c(1:(length(sample_index)/block_size)),size=length(sample_index),replace=TRUE))
    # i_temp = sample(1:k, max(groups), replace = TRUE)
    # for(iii in 1:max(groups)){
    #   index[which(groups==iii)]=i_temp[iii] 
    # }
    
    index=c()
    for(jjj in 1:k){
      index=c(index,c(rep(jjj,block_size)))
    }
    index = rep(index,ceiling(nrow(data)/length(index)))
  ...
  for(i in 1:k){
    ...
    bindex[index==i]=0
    bindex[bindex!=0]=1
    DOsigfactor <- (1+cumul_zeros2(bindex))
...
CV.KF.MV2 <- function(Y,ZZ,k,lambdavec,lambda2=0.001,dimX,plot=0,sweigths=1,eweigths=1,nf=dimX,beta0_given=FALSE,block_size=8){
  #what takes time is to compute the K matrix each time. Using the FWL trick
  #and having the k loop being the outter loop rather than lambda (as GA) function,
  #we can speed things up dramatically. However, CV interpretation will change.
  #Instead of finding the best model, it will find the best improvment wrt to constant
