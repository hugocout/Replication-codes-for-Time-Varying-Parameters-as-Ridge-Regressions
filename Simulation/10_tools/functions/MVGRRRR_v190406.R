MV.GRRRR <-function(X,Y,type,lambdavec=exp(linspace(-6,20,n=50)),lambda2=.1,
                 sweigths=1,oosX=1,kfold=5,silent=1,alpha=.5,nnlasso=0,olsprior=0,
                 adaptive=0,aparam=0,tol=10^(-2),maxit=15,starting_values=c(),CV.2SRR=FALSE,
                 lambdabooster=1,var.share=.8,override=0,id=3,homo.param=.75,orthoFac=FALSE,
  ...
  scalingfactor = array(1,dim=c(M,dim(X)[3]))
  sdy = 1:M
  
  for(j in 1:dim(X)[3]){
    for(m in 1:M){scalingfactor[m,j]=sd(Y[,m])/sd(X[m,,j])
    X[m,,j]=(X[m,,j])/sd(X[m,,j])
  }}
  
  for(m in 1:M){sdy[m]=sd(Y[,m])
  Y[,m] = Y[,m]/sdy[m]}
  dimX= dim(X)[3]+1 # + constant

  ZZ= array(0,dim=c(M,bigT,bigT*dimX))
  for(m in 1:M){ZZ[m,,] <- Zfun(X[m,,])} #Basis expansion
  ...
  #CV
    ...
    if(length(lambdavec)>1){
      ...
      cvlist <- CV.KF.MV(ZZ=ZZ[m,,],Y=Y[,m],dimX = dimX,lambdavec = lambdavec[l.subset],k=kfold,plot=abs(silent-1),sweigths=sweigths)
    }
      lambda1 <-cvlist$minimizer  #l.subset[1] #
      mse <- cvlist$minima
    }
    else{
      lambda1 = lambdavec[round(length(lambdavec)/2)]
      mse=100
    } 
      #cvlist$lambdas_het[m]
    
    #Final estimation
    grr<-dualGRR(ZZ[m,,],Y[,m],dimX,lambda1,olsprior = olsprior,lambda2=lambda2,sweigths=sweigths)
...
###### FACTOR TVP VAR #####
    ...
    Xkron = matrix(0,M*bigT,M*dimX)
    for(m in 1:M){Xkron[(((m-1)*bigT)+1):(m*bigT),(((m-1)*dimX)+1):(m*dimX)]=cbind(1,X[m,,])}
    ...
    repeat {
      ...
      NEWZ=c()
      for(m in 1:M){
        newZ=R_Lambda(X=X[m,,],newLambda = newLambda[((m-1)*dimX+1):(m*dimX),])
      ...
      for(m in 1:M){
        ...
        EW = ((mean(1))^(1-sv.param))*(garchstuff@sigma.t)^sv.param    #(svhat)^sv.param
        EW=append(EW[1],EW)
        #if(-step>=-max.step.cv){
        #svstuff <- CV.KF.MV(ZZ=Zfun(grra$yhat)[,c((bigT:(2*bigT-2)),2*bigT)],Y=yy_tilde,dimX = 1,lambdavec = lambdavec/100,
        ...
        #EW=(hpfilter(e^2, freq=24,type="frequency")$trend)^sv.param
        #print(sv$betas_grr[1,,])
        #EW=(1/sv$betas_grr[1,,])^(2*sv.param)
      ...
      for(jj in 1:r){newF[jj,]=newF[jj,]} #-mean(newF[jj,])
      orthoF= (pracma::sqrtm(orthomat)$Binv)%*%newF
      }
      else{orthoF=(newF)/sd(newF)} #-mean(newF)
      newF=orthoF
      }
      #for(jj in 1:r){newF[jj,]=newF[jj,]/sd(newF[jj,])} #-mean(newF[jj,])
      
      newZ=array(0,dim=c(M,bigT,r*dimX))
      for(m in 1:M){newZ[m,,]= R_f2(f=newF,dimX=dimX,Z=ZZ[m,,])} #MAKE THIS A LOOP FOR AN ARRAY
      ...
      #ADJUST THIS FUNCTION SO IT TAKES THE NEWZ ARRAY
      for(m in 1:M){
      if(step>max.step.cv){lambda.glmnet=old_lambdas_m[m]}
      gammareg= gamma_reg_f_VAR(y=Y[,m],X=cbind(X[m,,]),Z=newZ[m,,],eweigths=EWmat.std[,m],alpha=alpha_op,
      ...
      else{if(-sum(abs(newLambda))>-0.001*tol){r=0}}
      ...
      if(id==1){
      aa = UtoF((newU),var.share=var.share,override = override,id=id,vizualizar = abs(silent-1),foolproof=foolproof)
      if(orthoFac==TRUE){
      newF =aa$f
      newLambda = aa$Lambda
      }
  ...
  if(length(oosX)>1){
    ...
    fcast=1:M
    for(m in 1:M){
    fcast[m] <-  BETAS_VARF[m,,dim(BETAS_VARF)[3]] %*% as.matrix(append(1,oosX[m,]))  #basically the last beta_t
    }
  }
