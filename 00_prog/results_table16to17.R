...
load(paste(paths$out,'/table16to18/TVPfcst_111.Rdata',sep=''))
dim_forecast <- dim(forecast)
dim_forecast[2] <- 4
old_err_tab16 = array(NA,dim=c(dim_forecast,4))
...
for(mod in 1:4){
  for(v in 1:5){
    for(h in c(1,2,4)){
      ...
      old_err_tab16[,h,v,,mod]=forecast[,h_pos,v,]
      
      # Store table 17 results
      old_err_tab17[,h,v,,mod]=0.5*forecast[,h_pos,v,]+0.5*t(repmat(forecast[,h_pos,v,1],4,1))
